...
//656x368
bool getScale(const cv::Mat& im, cv::Size baseSize, float* scale, int* pad_x, int* pad_y){
    ...
    {
        {
            ...
            for (auto pair = 0u; pair < pairs.size(); pair += 2)
            {
                ...
                if (keypoints[index1 + 2] > threshold && keypoints[index2 + 2] > threshold)
                {
                    ...
                    const cv::Point keypoint1{ intRound((keypoints[index1] - pad_x) * scale), intRound((keypoints[index1 + 1] - pad_y) * scale) };
                    const cv::Point keypoint2{ intRound((keypoints[index2] - pad_x) * scale), intRound((keypoints[index2 + 1] - pad_y) * scale) };
            ...
            for (auto part = 0; part < numberKeypoints; part++)
            {
                ...
                if (keypoints[faceIndex + 2] > threshold)
                {
                    ...
                    const cv::Scalar color{ colors[(colorIndex+2) % numberColors],
                        colors[(colorIndex + 1) % numberColors],
                        colors[(colorIndex + 0) % numberColors] };
                    const cv::Point center{ intRound((keypoints[faceIndex] - pad_x) * scale), intRound((keypoints[faceIndex + 1] - pad_y) * scale) };
...
void renderPoseKeypointsCpu(cv::Mat& frame, const std::vector<float>& poseKeypoints, std::vector<int> keyshape,
    const float renderThreshold, float scale, int pad_x, int pad_y, const bool blendOriginalFrame)
{
    // Background
    ...
    renderKeypointsCpu(frame, poseKeypoints, keyshape, pairs, POSE_COCO_COLORS_RENDER, thicknessCircleRatio,
        thicknessLineRatioWRTCircle, renderThreshold, scale, pad_x, pad_y);
}
