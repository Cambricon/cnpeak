...
#ifndef CNPIPE_INFER_BATCH_SERVER_HPP
#define CNPIPE_INFER_BATCH_SERVER_HPP
...
class InferBatchServer {
public:
    static InferBatchServer* create_infer_batch_server(int server_num,
                                                    int thread_id, MMInfer* model, cnOfflineInfo* offineInfo);
    InferBatchServer(int server_id, MMInfer* model, cnOfflineInfo* offineInfo);
    ~InferBatchServer();
...
private:
    static vector<InferBatchServer*> insts_;
    int server_id_;
    int batch_size_;
